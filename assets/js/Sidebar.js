class Sidebar {
  ...
  openSidebarByIconsWithoudDropdown(
    ...
    icones.forEach((icon) => {
      icon.addEventListener('click', () => {
        this.openSidebar(sidebar, headerMain, iconMenu);
  ...
  /* ABRE O MENU */
  openSidebar(sidebar, headerMain) {
  ...
  dropdown(
    ...
    btnDropdown.forEach((button, index) => {
      button.addEventListener('click', () => {
        ...
        this.openSidebar(sidebar, headerMain);
      });
    });
  }
  ...
  closeSideDropdown(sideDropdowns) {
    const openedDropdowns = (sideDropdown) =>
      sideDropdown.style.display == 'block';
    const closeDropdowns = (sideDropdown) =>
  ...
  /* FECHA OS DROPDOWNS LATERAIS ABERTOS QUANDO CLICA EM UMA OPÇÃO DO MENU SEM DROP NO MESMO MODULO */
